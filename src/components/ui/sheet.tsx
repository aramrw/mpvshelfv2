...
export const sheetVariants = cva(
	"fixed z-50 gap-4 bg-primary shadow-lg transition ease-in-out data-[expanded]:animate-in data-[closed]:animate-out data-[expanded]:duration-200 data-[closed]:duration-200",
	{
		variants: {
			side: {
	...
	return (
		<DialogPrimitive.Portal>
			<DialogPrimitive.Overlay
				class={cn(
					"fixed inset-0 z-50 bg-primary/5 data-[expanded]:animate-in data-[closed]:animate-out data-[closed]:fade-out-0 data-[expanded]:fade-in-0",
				)}
			/>
			...
			>
				{local.children}
				<DialogPrimitive.CloseButton class="h-full text-secondary mr-2 rounded-sm opacity-70 transition-[opacity,box-shadow] hover:opacity-100 focus:outline-none disabled:pointer-events-none">
					<svg
