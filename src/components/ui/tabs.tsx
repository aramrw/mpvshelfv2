...
export const TabsList = <T extends ValidComponent = "div">(
	...
	return (
		<TabsPrimitive.List
			class={cn(
				"border-2 bg-popover shadow-md relative flex w-full p-2 text-muted-foreground data-[orientation=vertical]:flex-col data-[orientation=horizontal]:items-center data-[orientation=vertical]:items-stretch",
				local.class,
			)}
			{...rest}
		/>
...
export const TabsContent = <T extends ValidComponent = "div">(
	...
	return (
		<TabsPrimitive.Content
			class={cn(
				"w-full transition-shadow duration-200 focus-visible:outline-none focus-visible:ring-[1.5px] focus-visible:ring-ring focus-visible:ring-offset-2 focus-visible:ring-offset-background data-[orientation=horizontal]:mt-2 data-[orientation=vertical]:ml-2",
				local.class,
			)}
			{...rest}
		/>
...
export const TabsTrigger = <T extends ValidComponent = "button">(
	...
	return (
		<TabsPrimitive.Trigger
			class={cn(
				"peer relative z-10 inline-flex h-7 w-full items-center justify-center whitespace-nowrap rounded-sm px-3 py-1 text-sm font-medium outline-none transition-colors disabled:cursor-not-allowed  disabled:opacity-50 data-[selected]:text-foreground",
				local.class,
			)}
			{...rest}
		/>
...
const tabsIndicatorVariants = cva(
	"absolute transition-all duration-200 outline-none",
	{
		variants: {
			variant: {
				block:
					"data-[orientation=horizontal]:bottom-1 data-[orientation=horizontal]:left-0 data-[orientation=vertical]:right-1 data-[orientation=vertical]:top-0 data-[orientation=horizontal]:h-[calc(100%-0.5rem)] data-[orientation=vertical]:w-[calc(100%-0.5rem)] bg-background shadow rounded-sm peer-focus-visible:ring-[1.5px] peer-focus-visible:ring-muted peer-focus-visible:ring-offset-popover peer-focus-visible:ring-offset-red-500 peer-focus-visible:outline-none",
				underline:
